...
<template>
  <div style="height: 100%;">
    <div style="margin-bottom: 5px;margin-left: 5px; height: 5%">
      ...
      <el-button class="add" @click="addRecord" size="small">新增原材料</el-button>
    ...
    <el-dialog title="原材料详情" :visible.sync="dialogFormVisible1" style="height: 95vh; overflow: hidden" width="55%" center  >
        <el-form :model="form1" style="overflow-y: auto; height: 50vh;width: 50vw;">
          <el-form-item label="原材料类别编码" :label-width="formLabelWidth" required>
            <el-col :span="8">
              <el-input v-model="form1.mcategoryId" :readonly="true" autocomplete="true"></el-input>
            </el-col>
          </el-form-item>
          <el-form-item label="原材料类别名称" :label-width="formLabelWidth" required>
            <el-col :span="8"> 
              <el-input v-model="form1.mcategoryName"></el-input>
            </el-col>
          </el-form-item>                    
      ...
      <el-dialog title="原材料详情" :visible.sync="dialogFormVisible2" style="height: 95vh; overflow: hidden" width="55%" center  >
        <el-form :model="form2" style="overflow-y: auto; height: 50vh;width: 50vw;">
          <el-form-item label="原材料类别编码" :label-width="formLabelWidth" required>
            <el-col :span="8">
              <el-input v-model="form2.mcategoryId" autocomplete="true"  @blur="validateInput" ></el-input>
            </el-col>
          </el-form-item>
          <el-form-item label="原材料类别名称" :label-width="formLabelWidth" required>
            <el-col :span="8">
              <el-input v-model="form2.mcategoryName"></el-input>
            </el-col>
          </el-form-item>    
  ...
  methods: {
    ...
    loadPostlistPageC1() {
      ...
      treeLoad(tree, treeNode, resolve) {
        ...
        this.currentParentId = tree.mcategoryId
    // 在之前声明的全局变量中，增加一个key为 本条数据的id，id可替换为你数据中的任意唯一值
        this.tableTreeRefreshTool[tree.mcategoryId] = {}
      // 重要！保存resolve方法，以便后续使用
        this.tableTreeRefreshTool[tree.mcategoryId].resolve = resolve
      // 记录展开次数，具体作用后续介绍
        this.tableTreeRefreshTool[tree.mcategoryId].expandCount = 0
        setTimeout(() => {
          console.log("treeLoad",this.mcategoryId,this.mcategoryName,tree, treeNode, resolve);
          this.$axios.post(this.$httpUrl+'/raw-materials-category/listPageC1',{
          mcategoryName:this.mcategoryName,
          mcategoryId:this.mcategoryId,
          parentnodeId:tree.mcategoryId,
        }).then(res => res.data).then(res =>{
          console.log("post",res,res.data);
          if(res.code==200){
    ...
    handleExpandChange (row, expanded) {
      ...
      // 获取到之前保存的全局变量
      const curr = this.tableTreeRefreshTool[row.mcategoryId]
      // 展开次数 +1
      curr.expandCount++
