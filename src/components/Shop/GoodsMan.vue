...
<!-- 新增 -->
     <el-dialog title="新增商品" :visible.sync="dialogFormVisible2" style="height: 95vh; overflow: hidden" width="53%" center  >
...
export default {
    ...
    data() {
        return {
            ...
            itemKey: 0,
            referenid: JSON.parse(sessionStorage.getItem('CurUser')).shopId,
            productTable: [],
            multipleSelection: [],
            pageNum: 1,
            pageSize: 5,
    ...
    methods: {
      ...
      // http://localhost:8098/embroidery-carrier-categories/listPageC2
        ...
        loadPostlistPageC1() {
            this.$axios.post(this.$httpUrl + '/products/listPageC1',{
        ...
        handleDelete(index, row) {
            ...
            this.$axios.get(this.$httpUrl+'/products/delete?=productId'+row.productId,).then(res=>res.data).then(res=>{
            console.log(res);
            if(res.code=="200"){
                alert("删除成功");
                this.loadPostlistPageC1();
        ...
        newSave2(){
      console.log('newSave2',this.form2,this.shopId);
        this.dialogFormVisible2 = false;
        const data={...this.form2,referenid:this.referenid,image: Math.floor(Math.random() * 14) + 1};
        this.$axios.post(this.$httpUrl + '/products/saveOrMod', data).then(res => {
        console.log(res);
        if (res.status == "200") {
          alert("新增成功");
          this.loadPostlistPageC1();
