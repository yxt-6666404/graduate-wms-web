...
<template>
  <div style="height: 100%;">
    <div style="margin-bottom: 5px;margin-left: 5px; height: 5%">
      ...
      <el-input type="text" autofocus="true" placeholder="请输刺绣流派类别名称" v-model="categoryName" class="input-with-select" style="width: 200px;margin-left: 5px;" suffix-icon="el-icon-search" clearable size="small" 
    ...
    <el-table
      ...
      <el-table-column
        ...
        label="祖先类别">
        <template slot-scope="scope">
          <span>{{ scope.row.ancester !=='null' ? scope.row.ancester : '' }}</span>
        </template>
      </el-table-column>
      <el-table-column
  ...
  methods: {
    ...
    loadPostlistPageC1() {
      ...
      treeLoad(tree, treeNode, resolve) {
        ...
        this.currentParentId = tree.categoryId
    // 在之前声明的全局变量中，增加一个key为 本条数据的id，id可替换为你数据中的任意唯一值
        this.tableTreeRefreshTool[tree.categoryId] = {}
      // 重要！保存resolve方法，以便后续使用
        this.tableTreeRefreshTool[tree.categoryId].resolve = resolve
      // 记录展开次数，具体作用后续介绍
        this.tableTreeRefreshTool[tree.categoryId].expandCount = 0
        setTimeout(() => {
          console.log("treeLoad",this.categoryId,this.categoryName,tree, treeNode, resolve);
          this.$axios.post(this.$httpUrl+'/embroidery-category/listPageC1',{
          categoryName:this.categoryName,
          categoryId:this.categoryId,
          parentnodeId:tree.categoryId,
        }).then(res => res.data).then(res =>{
          console.log("post",res,res.data);
          if(res.code==200){
    ...
    handleExpandChange (row, expanded) {
      ...
      // 获取到之前保存的全局变量
      const curr = this.tableTreeRefreshTool[row.categoryId]
      // 展开次数 +1
      curr.expandCount++
