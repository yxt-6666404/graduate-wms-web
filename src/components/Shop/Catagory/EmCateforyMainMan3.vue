...
<template>
  <div style="height: 100%;">
    <div style="margin-bottom: 5px;margin-left: 5px; height: 5%">
      ...
      <el-input type="text" autofocus="true" placeholder="请输载体类别名称" v-model="ccategoryName" class="input-with-select" style="width: 200px;margin-left: 5px;" suffix-icon="el-icon-search" clearable size="small" 
      ...
      <el-button class="add" @click="addRecord" size="small">新增刺绣载体</el-button>
    ...
    <el-dialog title="刺绣载体详情" :visible.sync="dialogFormVisible1" style="height: 95vh; overflow: hidden" width="55%" center  >
        <el-form :model="form1" style="overflow-y: auto; height: 50vh;width: 50vw;">
          <el-form-item label="刺绣载体类别编码" :label-width="formLabelWidth" required>
            <el-col :span="8">
              <el-input v-model="form1.ccategoryId" :readonly="true" autocomplete="true"></el-input>
            </el-col>
          </el-form-item>
          <el-form-item label="刺绣载体类别名称" :label-width="formLabelWidth" required>
            <el-col :span="8"> 
              <el-input v-model="form1.ccategoryName"></el-input>
            </el-col>
          </el-form-item>                    
      ...
      <el-dialog title="刺绣载体详情" :visible.sync="dialogFormVisible2" style="height: 95vh; overflow: hidden" width="55%" center  >
        <el-form :model="form2" style="overflow-y: auto; height: 50vh;width: 50vw;">
          <el-form-item label="刺绣载体类别编码" :label-width="formLabelWidth" required>
            <el-col :span="8">
              <el-input v-model="form2.ccategoryId" autocomplete="true"  @blur="validateInput" ></el-input>
            </el-col>
          </el-form-item>
          <el-form-item label="刺绣载体类别名称" :label-width="formLabelWidth" required>
            <el-col :span="8">
              <el-input v-model="form2.ccategoryName"></el-input>
            </el-col>
          </el-form-item>    
...
export default {
  ...
  methods: {
    ...
    loadPostlistPageC1() {
      ...
      this.loading = true;
      this.$axios.post(this.$httpUrl+'/embroidery-carrier-categories/listPageC1',{
      ccategoryName:this.ccategoryName,
      ccategoryId:this.ccategoryId,
      ...
      treeLoad(tree, treeNode, resolve) {
        ...
        this.currentParentId = tree.ccategoryId
    // 在之前声明的全局变量中，增加一个key为 本条数据的id，id可替换为你数据中的任意唯一值
        this.tableTreeRefreshTool[tree.ccategoryId] = {}
      // 重要！保存resolve方法，以便后续使用
        this.tableTreeRefreshTool[tree.ccategoryId].resolve = resolve
      // 记录展开次数，具体作用后续介绍
        this.tableTreeRefreshTool[tree.ccategoryId].expandCount = 0
        setTimeout(() => {
          console.log("treeLoad",this.ccategoryId,this.ccategoryName,tree, treeNode, resolve);
          this.$axios.post(this.$httpUrl+'/embroidery-carrier-categories/listPageC1',{
          ccategoryName:this.ccategoryName,
          ccategoryId:this.ccategoryId,
          parentnodeId:tree.ccategoryId,
        }).then(res => res.data).then(res =>{
          console.log("post",res,res.data);
          if(res.code==200){
    ...
    handleExpandChange (row, expanded) {
      ...
      // 获取到之前保存的全局变量
      const curr = this.tableTreeRefreshTool[row.ccategoryId]
      // 展开次数 +1
      curr.expandCount++
    ...
    handleDelete(index, row) {
      ...
      this.$axios.get(this.$httpUrl+'/embroidery-carrier-categories/delete?ccategoryId='+row.ccategoryId,).then(res=>res.data).then(res=>{
        console.log(res);
        if(res.code=="200"){
          alert("删除成功");
          this.loadPostlistPageC1();
    ...
    newSave1(){
      console.log('newSave1',this.form1);
        ...
        this.$axios.post(this.$httpUrl + '/embroidery-carrier-categories/saveOrMod', this.form1).then(res => {
        console.log(res);
        if (res.status == "200") {
          alert("修改成功");
          this.loadPostlistPageC1();
    ...
    validateInput() {
      const value = this.form2.ccategoryId.trim(); // 获取输入的值并去除首尾空格
      ...
      if (!regex.test(value)) {
        alert('输入内容必须以"C"并加上一个大写字母为开头');
      }
    },
    newSave2(){
      console.log('newSave2',this.form2);
        this.dialogFormVisible2 = false;
        this.$axios.post(this.$httpUrl + '/embroidery-carrier-categories/saveOrMod2', this.form2).then(res => {
        console.log(res);
        if (res.status == "200") {
          alert("新增成功");
          this.loadPostlistPageC1();
